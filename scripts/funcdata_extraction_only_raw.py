...
from ccmodels.preprocessing.selectivity import orientation_extractor_v2, von_mises, von_mises_single, is_selective, cell_area_identifiers, fpd_assignment, identify_multiscan
...
for mult_id in tqdm(multiscan_ids, desc = f'Extracting multiscan neurons data') :
    ...
    all_acts = pd.concat(scan_acts, axis = 0)
    data.append(all_acts)

    
#Save the data
data_df =  pd.concat(data, axis = 0)
data_df.to_pickle(f'./data/in_processing/activities/activities_multiscan.pkl')
